 ...
 * 默认受峰时窗口约束的提供方路由。
 *
 * 峰谷定价只存在于 DeepSeek 官方路由（DSH 里是 `deepseek-official`）。别的提供方
 * ——自建网关、第三方中转、别家的模型——都不按峰谷计费，峰时没有任何理由拦它们。
 * 换提供方或换 id 时改 `peakProviders` 配置即可，不必改代码。
 */
export const DEFAULT_PEAK_PROVIDERS = Object.freeze(['deepseek-official']);
...
/**
 * 关掉 LAA 之后、把压着的轮次交出去时投递的提示词。
 *
 * 与 {@link RESUME_FRAMING} 分开是有意的：这里是「人把 LAA 关了」，不是「谷时到了」。
 * 措辞必须对得上，否则模型会以为自己正处在谷时窗口里。
 */
export const DISABLE_FRAMING = [
  '[LAA MODE — TURNED OFF]',
  'LAA mode was switched off for this session, so it is no longer held during DeepSeek peak hours.',
  'The turn that had been stopped at the peak boundary is released now; continue from the current session history, workspace state, and tool results.',
  'Input that was held during the peak window is delivered right behind this message, in order.',
  'Do not restate what is already finished: resume at the first unfinished step and tell the user in one line what you are resuming.',
].join('\n');
...
export function normalizeConfig(raw) {
  ...
  return {
    ...
    /**
     * 哪些提供方路由算「按峰谷计费」：只有它们的请求会在峰时被拦下。数组里放路由 id；
     * 显式给空数组表示「一个都不拦」（峰时约束失效，谷时的重放语义仍然保留）。值不合法
     * （不是字符串数组）时退回 {@link DEFAULT_PEAK_PROVIDERS}。
     */
    peakProviders: Array.isArray(config.peakProviders)
      && config.peakProviders.every((provider) => typeof provider === 'string' && provider.trim().length > 0)
      ? config.peakProviders.map((provider) => provider.trim())
      : DEFAULT_PEAK_PROVIDERS,
    /**
     * 峰谷切换时浏览器通知的策略：`enabled`（默认，只有开着 LAA 的会话才播报）、
     * `always`（任何会话都播报）、`off`（不播报）。它只影响浏览器端那一条通知，
     * 开关上的切换动画不受影响。
     */
    notify: config.notify === 'always' || config.notify === 'off' ? config.notify : 'enabled',
...
export function createLaaRuntime(ctx, rawConfig, deps = {}) {
  ...
  //#region 提供方
  /**
   * 一个 agent 的请求当前实际走的路由 id。
   *
   * 三级回退，从最贴近事实到最兜底：
   * 1. 会话最近一次请求的记录头（`session.requestHeader()`）——中途换过模型的话，
   *    只有这里反映当前真正在用的路由，`agent.options` 还停在 agent 创建时那一个；
   * 2. agent 创建时的路由（`agent.options.provider`）；
   * 3. 部署默认路由（`ctx.agentDefaultModel.currentSelection()`）。
   * @param agent - 实时 agent。
   * @returns 提供方路由 id；三级都读不到时返回 undefined。
   */
  function providerOf(agent) {
    const logged = typeof agent?.session?.requestHeader === 'function' ? agent.session.requestHeader() : undefined;
    const fromLog = logged?.config?.provider;
    if (typeof fromLog === 'string' && fromLog.length > 0) return fromLog;
    const created = agent?.options?.provider;
    if (typeof created === 'string' && created.length > 0) return created;
    const defaults = ctx.get('agentDefaultModel');
    const fallback = defaults !== undefined && typeof defaults.currentSelection === 'function' ? defaults.currentSelection() : undefined;
    const provider = fallback?.provider;
    return typeof provider === 'string' && provider.length > 0 ? provider : undefined;
  }

  /**
   * 这个 agent 的请求是否按峰谷计费——也就是峰时到底该不该拦它。
   *
   * 只有 {@link DEFAULT_PEAK_PROVIDERS}（默认 `deepseek-official`，可用 `peakProviders`
   * 配置）里的提供方才受峰时窗口管辖；别的提供方没有峰谷定价，峰时照跑。
   * 读不到路由时按「是」处理（保守）：拦错了只是多等一会儿，漏拦了会真的按峰时价格计费。
   * @param agent - 实时 agent。
   * @returns 是否受峰时约束。
   */
  function isPeakBilled(agent) {
    const provider = providerOf(agent);
    if (provider === undefined) return true;
    return config.peakProviders.includes(provider);
  }
  //#endregion
  ...
  function governedSessionOf(agent) {
    ...
    /* 只有按峰谷计费的提供方受管辖：别的模型峰时照跑，一个请求都不拦。 */
    if (!isPeakBilled(agent)) return undefined;
  ...
  /**
   * 把一个会话的遗留工作交还给它的 agent：先续跑被中断的轮次，再按原顺序重放被拦下的
   * 输入。只有真正投递成功的部分才会被清除。
   *
   * @param agent - 这个会话自己的实时 agent。
   * @param sessionId - 会话 id（遗留工作就记在它名下）。
   * @param options.framing - 续跑提示词；谷时用 {@link RESUME_FRAMING}，关掉 LAA 时用
   *   {@link DISABLE_FRAMING}。
   * @param options.reason - 写进日志的原因短语。
   * @returns 这次投递了多少：`resumed`（是否发了续跑提示词）与 `delivered`（重放的输入条数）。
   */
  function release(agent, sessionId, options = {}) {
    const entry = store.get(sessionId);
    if (!entry.suspended && entry.deferred.length === 0) return { resumed: false, delivered: 0 };
    if (entry.goalBlocked) rearmGoal(agent);
    const framing = options.framing ?? RESUME_FRAMING;
    const reason = options.reason ?? 'off-peak began';
    const delivered = [];
    let resumed = false;
    try {
      if (entry.suspended) {
    ...
    if (resumed || delivered.length > 0) {
      ctx.logger.info(`dsh-laa: ${reason}; session "${sessionId}" resumed (${delivered.length} deferred input(s))`);
    }
    return { resumed, delivered: delivered.length };
  }

  /**
   * 关掉 LAA 时把一棵会话树上仍被压着的遗留工作立刻交出去。
   *
   * 遗留工作按会话各自记账（见模块头部），所以整棵树都要看一遍：父会话的输入回到父
   * 代理，子会话的输入回到子代理。没有实时 agent 的会话交不出去，留在状态里等它下次
   * 变成实时会话（{@link adopt}）——输入一条都不会丢。
   *
   * @param anchor - 顶层会话 id。
   * @returns `sessions`（真正交出去的会话数）、`delivered`（输入条数）、`waiting`（还在等的会话数）。
   */
  function releaseHeld(anchor) {
    const summary = { sessions: 0, delivered: 0, waiting: 0 };
    for (const sessionId of store.pendingIds()) {
      if (anchorOf(sessionId) !== anchor) continue;
      const agent = ctx.agents.get(sessionId);
      if (agent === undefined) {
        summary.waiting += 1;
        continue;
      }
      const outcome = release(agent, sessionId, { framing: DISABLE_FRAMING, reason: 'LAA was switched off' });
      if (outcome.resumed || outcome.delivered > 0) summary.sessions += 1;
  ...
  return {
    ...
    /**
     * 一个新 agent 出现：把等待中的工作立刻交给它。
     *
     * 模式开着：只有谷时才交（峰时继续压着）。模式关着：攒下的输入没有任何理由继续
     * 等待，直接发出去——包括「关掉 LAA 时这个会话还没有实时 agent」的那种情况。
     *
    ...
    adopt(agent) {
      ...
      if (enabled && phaseNow() !== VALLEY) return;
      release(agent, sessionId, enabled ? {} : { framing: DISABLE_FRAMING, reason: 'LAA was switched off' });
    },
     ...
     *
     * **关掉就立刻把压着的工作发出去**：模式一旦关闭，那些输入没有任何理由继续等
     * 谷时；没有实时 agent 的会话交不出去，留在状态里等它下次运行（见 {@link releaseHeld}）。
     *
     * @returns `entry`（锚点那条记录）与 `released`（这次交出去多少）。
     */
    setEnabled(sessionId, enabled) {
      const anchor = anchorOf(sessionId);
      const entry = store.mutate(anchor, (previous) => ({ ...previous, enabled, updatedAt: clock() }));
      const released = enabled ? { sessions: 0, delivered: 0, waiting: 0 } : releaseHeld(anchor);
    ...
    snapshot(sessionId, now = clock()) {
      ...
      /* 提供方只从实时 agent 读得到；会话冷着的时候按「按峰谷计费」处理。 */
      const live = typeof ctx.agents.get === 'function' ? ctx.agents.get(sessionId) : undefined;
      const provider = live === undefined ? undefined : providerOf(live);
      return {
        sessionId,
        enabled: isEnabled(sessionId),
        /* 这个会话当前走的路由（读不到时为 null）与它是否受峰时约束。 */
        provider: provider === undefined ? null : provider,
        peakBilled: live === undefined ? true : isPeakBilled(live),
