/**
 * dsh-laa 的持久状态：每个会话的 LAA 开关、峰时被拦下等待谷时重放的输入，以及
 * 子会话的谱系（它属于哪个顶层会话）。
 *
...
export function createStore({ filePath, logger, maxDeferred = DEFAULT_MAX_DEFERRED, flushDelayMs = 250 }) {
   ...
   * 子会话 -> 它所属的父会话。
   *
   * 会话创建时的父子关系是**不可变事实**（DSH 把它写在会话 header 的
   * `parentSession` 上），所以这张表只会被追加、不会过期：它让插件在子会话
   * 冷着（没有实时 agent、没有实时会话对象）的时候也能沿谱系找到顶层会话。
  ...
  function load() {
    ...
    /* 谱系是后加的字段：老状态文件没有它，读作「什么都不知道」而不是损坏。 */
    if (parsed.lineage !== null && typeof parsed.lineage === 'object') {
      for (const [childId, parentId] of Object.entries(parsed.lineage)) {
        if (typeof childId !== 'string' || childId.length === 0) continue;
        if (typeof parentId !== 'string' || parentId.length === 0 || parentId === childId) continue;
        lineage.set(childId, parentId);
      }
    }
  }
  ...
  function writeNow() {
    ...
    const document = {
      ...
      lineage: Object.fromEntries(lineage),
    };
  ...
  return {
    ...
    /**
     * 记录一条谱系：`childId` 是 `parentId` 的子会话。
     *
     * 只记事实、不做推导（祖先由运行时沿这张表向上走），重复记录同一对是空操作
     * ——于是它可以被放在每次读取会话元数据的热路径上而不会反复触发落盘。
     * @returns 是否真的写入了新的一对。
     */
    setLineage(childId, parentId) {
      if (typeof childId !== 'string' || childId.length === 0) return false;
      if (typeof parentId !== 'string' || parentId.length === 0 || parentId === childId) return false;
      if (lineage.get(childId) === parentId) return false;
      lineage.set(childId, parentId);
      schedule();
      return true;
    },

    /** 一个会话的直接父会话；插件从没见过这条谱系时返回 undefined。 */
    lineageOf(sessionId) {
