...
function execute(runtime, agent, rawInput) {
  ...
  switch (action) {
    ...
    case 'off': {
      /* 关闭会把压着的工作立刻发出去，命令要把这件事报告清楚。 */
      const { released } = runtime.setEnabled(sessionId, false);
      /* 子会话没有自己的模式：这里的关闭同时关掉了父会话，得说清楚。 */
      const anchor = runtime.anchorOf(sessionId);
      const inherited = typeof anchor === 'string' && anchor.length > 0 && anchor !== sessionId
        ? `\n这是子会话：父会话 ${anchor} 的开关已一并关闭。`
        : '';
      const flushed = released.sessions > 0
        ? `\n压着的工作已经发出去：${released.sessions} 个会话${released.delivered > 0 ? `、${released.delivered} 条输入` : ''}。`
        : '';
      const waiting = released.waiting > 0
        ? `\n还有 ${released.waiting} 个会话暂时没有实时 agent，它们的输入会在下次运行时投递。`
        : '';
      return reply('success', `LAA 模式已关闭。${inherited}${flushed}${waiting}`);
    }
    default:
